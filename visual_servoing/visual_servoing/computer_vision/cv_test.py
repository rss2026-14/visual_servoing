...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        arg = sys.argv[1]
        if arg == "cone":
            scores = test_all_algorithms(
                cone_csv_path, cone_template_path, cone_score_path)
        elif arg == "map":
            scores = test_all_algorithms(
                localization_csv_path, localization_template_path, localization_score_path, swap=True)
        elif arg == "citgo":
            scores = test_all_algorithms(
                citgo_csv_path, citgo_template_path, citgo_score_path)
        else:
        ...
        if args[0] in {"cone", "map", "citgo"} and args[1] in {"color", "template", "sift"}:
            if args[0] == "map":
                swap = True
            scores = test_algorithm(
                algo_dict[args[1]], data_dict[args[0]][0], data_dict[args[0]][1], swap=swap)
        else:
